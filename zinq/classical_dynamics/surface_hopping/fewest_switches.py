...
class FewestSwitches(SurfaceHopping):
    ...
    def _coef_propagator(self, H_eff: np.ndarray, TDC: np.ndarray, dt: float) -> np.ndarray:
        M = -TDC - 1j * H_eff[..., np.newaxis] * np.eye(H_eff.shape[1])

        M1 = dt / self.substeps * M
        M2 = M1 @ M1
        M3 = M1 @ M2
        M4 = M1 @ M3
    ...
    def _calc_probs(self, ensemble: Ensemble, TDC: np.ndarray, dt: float) -> np.ndarray:
        indices = np.arange(ensemble.ntraj)

        c_active = ensemble.c[indices, ensemble.states, None] 
        coupling = TDC[indices, ensemble.states, :]
    ...
    def _integrate_substeps(self, ensemble: Ensemble, TDC: np.ndarray, prop: np.ndarray, dt: float) -> tuple[np.ndarray, np.ndarray]:
        ...
        for _ in range(self.substeps):
            ...
            probs = self._calc_probs(ensemble, TDC, dt / self.substeps)
    ...
    def _prepare_propagator(self, V_a: np.ndarray, dt: float) -> tuple[np.ndarray, np.ndarray]:
        TDC = self.tdc.evaluate(self._U_history[0], self._U_history[1], dt)
        
        H_eff = V_a - np.mean(V_a, axis=1, keepdims=True)
        prop = self._coef_propagator(H_eff, TDC, dt)
    ...
    def jump(self, ensemble: Ensemble, pot: Potential, dt: float, time: float) -> None:
        ...
        TDC, prop = self._prepare_propagator(V_a, dt)

        jump_mask, target_states = self._integrate_substeps(ensemble, TDC, prop, dt)
