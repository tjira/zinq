...
\usepackage{setspace} % setting the line spacing
...
\AtBeginDocument{\onehalfspacing} % set line spacing to 1.5 at the beginning of document
...
\begin{equation}
U(t)=\sum_n \mathrm e^{-i t E_n} \ket{n}\bra{n}+\int_{\sigma_{\mathrm c}(H)} \mathrm e^{-i t E}\,\ket{E}\bra{E}\,\mathrm{d}E,
\end{equation}
where $E_n$ and $E$ denote discrete and continuous energy values, respectively. This expression shows explicitly how each energy component of the state picks up a phase factor $\mathrm e^{-i t E}$, while the norm
\begin{equation}
...
\begin{equation}
i\frac{\partial}{\partial{t}}\ket{\chi_I(\symbf{R},t)}=\left[T_\mathrm{n}(\symbf{R})+E_I(\symbf{R})\right]\ket{\chi_I(\symbf{R},t)}+\sum_JC_{IJ}(\symbf{R})\,\ket{\chi_J(\symbf{R},t)},
\end{equation}
...
\begin{equation}
i\frac{\partial}{\partial{t}}\ket{\tilde\chi_\alpha(\symbf{R},t)}=\tilde{T}_\mathrm{n}\ket{\tilde\chi_\alpha(\symbf{R},t)}+\sum_\beta\tilde{V}_{\alpha\beta}(\symbf{R})\ket{\tilde\chi_\beta(\symbf{R},t)},
\end{equation}
...
In the described methods, we work in a position representation $\Psi\left(\symbf{R},t\right):=\braket{\symbf{R}\vert\Psi\left(t\right)}$. More importantly, we expand the wavefunction $\ket{\Psi\left(t\right)}$ in a set of diabatic basis functions $\lbrace\tilde\phi_I\rbrace_{I=1}^N$ defined in Sec~\ref{sec:adiabatic_vs_diabatic_representation}, so that $\Psi\left(\symbf{R},t\right)$ is an N-component vector in a electronic diabatic subspace.
\section{Split-Operator Method}\label{sec:split_operator_method}
In the Split-Operator method, we can write a general solution of the \acrshort{tdse} as
\begin{equation}\label{eq:tdse_propagator}
\ket{\Psi\left(t\right)}=\symrm{e}^{-it H}\ket{\Psi\left(0\right)}.
\end{equation}
The action of the operator $\symrm{e}^{-it H}$ on the initial wavefunction $\ket{\Psi\left(0\right)}$ propagates the state from time $0$ to time $t$, carrying all the information about the time evolution of the system. Since $H$ is self-adjoint, the spectral theorem yields a unitary propagator, ensuring $\lvert\Psi(t)\rvert=\lvert\Psi(0)\rvert$. This exponential operator contains the complete description of how the system evolves with time. In practical applications, however, evaluating $\symrm{e}^{-it H}$ analytically is rarely straightforward for nontrivial Hamiltonians, which often is exactly the proble the Split-Operator method solves.
...
\begin{equation}
\symrm{e}^{-it H}\neq\symrm{e}^{-it\tilde{V}}\symrm{e}^{-it\tilde{T}_\mathrm{n}}.
\end{equation}
If such a factorization were possible, we could apply the exponential of $\tilde{V}$ in the position space and the exponential of $\tilde{T}_\mathrm{n}$ momentum space separately. To address this, we split the total propagation time $t$ into $n$ short steps $\Delta t$, so the full propagator becomes a product of short-time propagators as
\begin{equation}
U(t)=\symrm{e}^{-it H}=\left(\symbf{e}^{-i\Delta t H}\right)^n,\qquad t=n\Delta t
\end{equation}
...
\begin{equation}
\symrm{e}^{-i\Delta t H}=\symrm{e}^{-i\frac{\Delta t}{2}\tilde{V}}\symrm{e}^{-i\Delta t\tilde{T}_\mathrm{n}}\symrm{e}^{-i\frac{\Delta t}{2}\tilde{V}}+\mathcal{O}(\Delta t^3),
\end{equation}
...
\begin{equation}
\Psi\left(\symbf{R},t+\Delta t\right)=\symrm{e}^{-i\frac{\Delta t}{2}\tilde V}\mathcal{F}^{-1}\left\lbrace\symrm{e}^{-i\Delta t\tilde{T}_\mathrm{n}}\mathcal{F}\left\lbrace\symrm{e}^{-i\frac{\Delta t}{2}\tilde V}\Psi\left(\symbf{R},t\right)\right\rbrace\right\rbrace,
\end{equation}
...
\section{Crank--Nicolson Method}\label{sec:crank_nicolson_method}
...
Similar to real time propagation, we start with the \acrshort{tdse}, but this time we introduce imaginary time $\tau$ via $\tau=it$ to obtain the equation
...
\begin{equation}
i\symbf{\dot{c}}(t)=H_{\mathrm{eff}}\symbf{c}(t),\qquad H_{IJ}^{\mathrm{eff}}=E_I\delta_{IJ}-i\sigma_{IJ}
\end{equation}
