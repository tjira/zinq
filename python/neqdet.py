...
# set the print options for numpy and the random seed
...
def neqdet():
    ...
    potf = lambda r: np.array(eval(args.potential)).transpose(2, 0, 1);

    # define the function to evaluate the guess wavefunction on a specified grid
    psif = lambda r: np.array(list(map(lambda x: x * np.ones(args.points**ndim), eval(args.guess))), dtype=complex).T
    ...
    V, psi = potf(r), psif(r); shape = ndim * [args.points] + [psi.shape[1]]
    ...
    # print the acf
    axs[1, 0].plot(np.arange(-args.iterations, args.iterations + 1) * args.timestep, np.array(acf).real, label="$\Re(\mathrm{ACF})$")
    axs[1, 0].plot(np.arange(-args.iterations, args.iterations + 1) * args.timestep, np.array(acf).imag, label="$\Im(\mathrm{ACF})$")
    ...
    axs[0, 2].plot(time, np.array(position)[:, 0], color="tab:orange", label="$\Braket{\Psi|\hat{r_x}|\Psi}$")
    axs[1, 2].plot(time, np.array(momentum)[:, 0], color="tab:orange", label="$\Braket{\Psi|\hat{p_x}|\Psi}$")

    # plot the mean of the Bohmian trajectories
    axs[0, 2].plot(time, np.mean( trajs[:, :, 0], axis=0), "--", color="black", label="$\langle r_x\\rangle$")
    axs[1, 2].plot(time, np.mean(ptrajs[:, :, 0], axis=0), "--", color="black", label="$\langle p_x\\rangle$")
