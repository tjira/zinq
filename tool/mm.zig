...
pub fn parse(first: *[]const u8, second: *[]const u8, result: *[]const u8, allocator: std.mem.Allocator, h: *bool) !std.process.ArgIterator {
    ...
    while (argv.next()) |arg| {
        ...
        if (std.mem.eql(u8, arg, "-i") or std.mem.eql(u8, arg, "--input")) {
            first.* = argv.next() orelse return error.InvalidArgument;
            argc += 1;
            second.* = argv.next() orelse return error.InvalidArgument;
            argc += 1;
        }
...
pub fn main() !void {
    ...
    var argv = try parse(&first, &second, &result, allocator, &h);
    defer argv.deinit();
    if (h) return;
    ...
    {
        var A = try readRealMatrix(f64, first, allocator);
        defer A.deinit(allocator);

        var B = try readRealMatrix(f64, second, allocator);
        defer B.deinit(allocator);

        var C = try RealMatrix(f64).init(A.rows, B.cols, allocator);
        defer C.deinit(allocator);

        var timer_mm = try std.time.Timer.start();
