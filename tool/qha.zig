...
const qhaEntropy = zinq.trajectory_thermodynamics.qhaEntropy;

const Na = global_variables.Na;
const Eh = global_variables.Eh;

pub fn help(io: std.Io) !void {
    try print(io, "USAGE: zinq-qha [-s TRAJECTORY_XYZ] [-t TEMPERATURE] [-h]\n", .{});
}
...
pub fn main(init: std.process.Init) !void {
    ...
    try print(io, "CALCULATING THE QHA ENTROPY OF A TRAJECTORY - TRAJECTORY: {s}, TEMPERATURE: {d:.3}\n", .{ trajectory_path, temperature });

    {
        ...
        try print(io, "\nCALCULATING THE QHA ENTROPY: ", .{});

        const entropy = try qhaEntropy(f64, traj.positions, traj.masses, temperature, true, allocator);

        try print(io, "{f}\n", .{timer_entropy.untilNow(io, .real)});

        try print(io, "\nQHA ENTROPY: {d:.8} Eh/K = {d:.8} J/MOL/K\n", .{ entropy, entropy * Na * Eh });
    }
