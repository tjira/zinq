...
\hat{H}=\hat{T}+\hat{V}=-\frac{\hbar^2}{2m}\frac{\partial^2}{\partial x^2}\hat{I}+
\begin{pmatrix}
V_{11}(x) & V_{12}(x) \\
V_{21}(x) & V_{22}(x)
\end{pmatrix},
...
where $$\hat{T}$$ is the kinetic matrix in the momentum space, and $$\hat{V}$$ is the potential matrix in the position space. The matrix exponential is now simply a mathematical problem and is described [here](/zinq/appendix/mathematical_methods/matrix_exponential.html#matrix_exponential).
