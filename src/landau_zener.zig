...
pub fn Parameters(comptime T: type) type {
    return struct { energy_gaps: RingBufferArray(T), coefficient: *ComplexVector(T), time_step: T };
}

/// Struct holding the Landau-Zener surface hopping algorithm implementation.
pub fn LandauZener(comptime T: type) type {
    return struct {
        pub const ThreeStateVariant = enum { maximum_curvature, maximum_probability, nearest, normalied_probability };
        ...
        pub fn getJumpProbabilities(self: @This(), jump_probabilities: *RealVector(T), parameters: Parameters(T), current_state: usize) void {
            ...
            var maxddZ1: T = -std.math.inf(T);
            var minZ0: T = std.math.inf(T);
            ...
            for (0..jump_probabilities.len) |i| if (i != current_state) {
                ...
                const A = ddZ1 / 2;
                const B = (Z0 - Z2) / (2 * parameters.time_step);
                const C = Z1;

                const t0 = -B / (2 * A);
                const g = A * t0 * t0 + B * t0 + C;

                const veff = std.math.sqrt(g * ddZ1);
                const delta: T = 0.25 * std.math.pow(T, g, 2) / veff;

                var p = std.math.exp(-2 * std.math.pi * delta);
                if (std.math.isNan(p)) p = 0;
                ...
                const c0 = parameters.coefficient.at(current_state);
                const c1 = parameters.coefficient.at(i);
