...
pub fn OrbitFractalGenerator(comptime T: type) type {
    return struct {
        ...
        pub fn init(opt: anytype) @This() {
            ...
            var generator = @This(){ .fractal = fractal, .algorithm = opt.algorithm, .coloring = opt.coloring, .center = opt.center, .zoom = opt.zoom };
        ...
        pub fn density(self: @This(), canvas: *Image, fractal: anytype, coloring: anytype, allocator: std.mem.Allocator) !void {
            const hf: T = @floatFromInt(canvas.height);
            const wf: T = @floatFromInt(canvas.width);
            ...
            var data = try allocator.alloc(usize, canvas.height * canvas.width);
            defer allocator.free(data);

            @memset(data, 0);

            var orbit = try allocator.alloc(Complex(T), maxiter);
            defer allocator.free(orbit);
