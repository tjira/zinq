...
pub fn DensityFunctional(comptime T: type) type {
    return struct { exchange: ?[:0]const u8 = "LDA_X", correlation: ?[:0]const u8 = "LDA_C_VWN", exchange_correlation: ?[:0]const u8 = null, params: ?[]const T = null };
}
...
pub fn getFunctionalFamily(name: [:0]const u8) !enum { lda, gga, mgga } {
    ...
    const kind_map = .{ .{ "LDA_X", .lda }, .{ "LDA_C_CHACHIYO", .lda }, .{ "LDA_C_VWN", .lda } };
    ...
    inline for (kind_map) |entry| {
        ...
        std.log.err("FUNCTIONAL '{s}' NOT FOUND", .{name});
...
pub fn getFunctionalKind(name: [:0]const u8) !enum { exchange, correlation, exchange_correlation, kinetic } {
    ...
    const kind_map = .{ .{ "LDA_X", .exchange }, .{ "LDA_C_CHACHIYO", .correlation }, .{ "LDA_C_VWN", .correlation } };
...
pub fn computeExchangeCorrelationArray(comptime T: type, exc: *RealVector(T), cor: *RealVector(T), functional: DensityFunctional(T), rho: RealVector(T), comptime derivative: u32) !void {
    if (functional.exchange_correlation != null and (functional.exchange != null or functional.correlation != null)) {
        std.log.err("CANNOT SPECIFY BOTH EXCHANGE-CORRELATION AND SEPARATE EXCHANGE/CORRELATION FUNCTIONALS", .{});
        return error.InputError;
    }

    if (functional.exchange) |name| if (try getFunctionalKind(name) != .exchange) {
        std.log.err("FUNCTIONAL '{s}' IS NOT AN EXCHANGE FUNCTIONAL", .{name});
        return error.InputError;
    };

    if (functional.correlation) |name| if (try getFunctionalKind(name) != .correlation) {
        std.log.err("FUNCTIONAL '{s}' IS NOT A CORRELATION FUNCTIONAL", .{name});
        return error.InputError;
    };

    if (functional.exchange_correlation) |name| if (try getFunctionalKind(name) != .exchange_correlation) {
        std.log.err("FUNCTIONAL '{s}' IS NOT AN EXCHANGE-CORRELATION FUNCTIONAL", .{name});
        return error.InputError;
    };
    ...
    var exchange: ?ExchangeFunctional(T) = null;
    var correlation: ?CorrelationFunctional(T) = null;

    const exchange_map = [_]struct { name: []const u8, val: ExchangeFunctional(T) }{.{ .name = "LDA_X", .val = .{ .slater = .{} } }};
    ...
    if (functional.exchange) |x| {
        for (exchange_map) |entry| {
            if (std.ascii.eqlIgnoreCase(x, entry.name)) {
                exchange = entry.val;
                break;
            }
        } else {
    ...
    if (functional.correlation) |c| {
        for (correlation_map) |entry| {
            if (std.ascii.eqlIgnoreCase(c, entry.name)) {
                correlation = entry.val;
                break;
            }
        } else {
    ...
    if (functional.exchange_correlation) |c| {
        for (correlation_map) |entry| {
            ...
            std.log.err("EXCHANGE-CORRELATION FUNCTIONAL '{s}' NOT FOUND", .{c});
            return error.InputError;
        }
    }
...
pub fn ChachiyoCorrelation(comptime T: type) type {
    return struct {
        /// Evaluate function.
        pub fn evaluateD0(_: @This(), rho: T) T {
            const a: T = -0.01554535;
            const b: T = 20.4562557;
