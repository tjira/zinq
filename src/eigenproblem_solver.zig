...
pub fn diagonalizeHermitian(comptime T: type, A: anytype) !void {
    if (comptime @TypeOf(A.*) == RealMatrix(T)) {
        return try eigensystemHermitianJacobi(RealMatrix, T, A, null, A.*);
    } else if (comptime @TypeOf(A.*) == ComplexMatrix(T)) {
        return try eigensystemHermitianJacobi(ComplexMatrix, T, A, null, A.*);
...
pub fn eigensystemHermitian(comptime T: type, J: anytype, C: anytype, A: anytype) !void {
    if (comptime @TypeOf(A) == RealMatrix(T) and config.use_openblas) if (A.rows > 32) {
        return try openblas.dsyevd(T, J, C, A);
    };

    if (comptime @TypeOf(A) == RealMatrix(T)) {
        return try eigensystemHermitianJacobi(RealMatrix, T, J, C, A);
    } else if (comptime @TypeOf(A) == ComplexMatrix(T)) {
        return try eigensystemHermitianJacobi(ComplexMatrix, T, J, C, A);
...
pub fn eigensystemHermitianJacobi(comptime M: fn (comptime type) type, comptime T: type, J: *M(T), C: ?*M(T), A: M(T)) !void {
    ...
    const n = A.rows;
    var iter: usize = 0;
    if (J.data.ptr != A.data.ptr) try A.copyTo(J);
    ...
    while (J.offDiagonalFrobeniusNorm() > tol) : (iter += 1) for (0..n - 1) |i| for (i + 1..n) |j| {
        ...
        for (0..n) |k| {
            ...
            J.ptr(k, i).* = uk;
            J.ptr(i, k).* = if (comptime M == RealMatrix) uk else uk.conjugate();
            J.ptr(k, j).* = vk;
            J.ptr(j, k).* = if (comptime M == RealMatrix) vk else vk.conjugate();
        }

        J.ptr(i, i).* = if (comptime M == RealMatrix) a - t * g else Complex(T).init(a - t * g, 0);
        J.ptr(i, j).* = if (comptime M == RealMatrix) 0 else Complex(T).init(0, 0);
        J.ptr(j, j).* = if (comptime M == RealMatrix) b + t * g else Complex(T).init(b + t * g, 0);
        J.ptr(j, i).* = if (comptime M == RealMatrix) 0 else Complex(T).init(0, 0);
