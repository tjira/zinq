...
pub fn Options(comptime _: type) type {
    return struct {
        pub const Filter = enum { all, mersenne };
        pub const Mode = union(enum) {
            check: struct { number: []const u8, filter: Filter = .all },
            factorize: struct { number: []const u8 },
            generate: struct {
                pub const OutputFiles = struct { interval: ?u32 = null, path: []const u8 };
...
pub fn checkMode(comptime T: type, opt: anytype, enable_printing: bool, allocator: std.mem.Allocator) !Output(T) {
    ...
    try number_big.setString(10, opt.number);
    const number = if (comptime T == std.math.big.int.Managed) number_big else try number_big.toInt(T);
...
pub fn factorizeMode(comptime T: type, opt: anytype, enable_printing: bool, allocator: std.mem.Allocator) !Output(T) {
    ...
    var prime_numbers = try allocator.alloc(std.math.big.int.Managed, factors.len);
    defer allocator.free(prime_numbers);
...
pub fn generateMode(comptime T: type, opt: anytype, enable_printing: bool, allocator: std.mem.Allocator) !Output(T) {
    ...
    try start_big.setString(10, opt.start);
    try start_big.addScalar(&start_big, -1);
    ...
    var prime_numbers: []T = try allocator.alloc(T, max_output);
    defer allocator.free(prime_numbers);
    ...
    for (0..opt.count) |i| {
        ...
        if ((i + 1) % max_output == 0) if (opt.output != null) {
            ...
            var buf_end: [32]u8 = undefined;

            const start_i_str = try std.fmt.bufPrint(&buf_start, "{d}", .{i + 2 - max_output});
            const end_i_str = try std.fmt.bufPrint(&buf_end, "{d}", .{i + 1});
...
pub fn factorize(p: anytype, allocator: std.mem.Allocator) ![]@TypeOf(p) {
    const T = @TypeOf(p);
    const ap = T == std.math.big.int.Managed;

    var one = if (comptime ap) try std.math.big.int.Managed.initSet(allocator, 1) else @as(T, 1);
    defer if (comptime ap) one.deinit();

    var two = if (comptime ap) try std.math.big.int.Managed.initSet(allocator, 2) else @as(T, 2);
    defer if (comptime ap) two.deinit();

    var rem = if (comptime ap) try std.math.big.int.Managed.initSet(allocator, 0) else @as(T, 0);
    defer if (comptime ap) rem.deinit();

    var pfd = if (comptime ap) try std.math.big.int.Managed.initSet(allocator, 0) else @as(T, 0);
    defer if (comptime ap) pfd.deinit();

    if (if (comptime ap) p.order(two) == std.math.Order.lt else p < 2) return &[_]T{};

    var factors: std.ArrayList(T) = .empty;

    var n = if (comptime ap) try p.clone() else p;
    var pd = if (comptime ap) try std.math.big.int.Managed.initSet(allocator, 0) else @as(T, 0);

    while (if (comptime ap) !n.eql(one) else n != 1) {
        var pd_next = try nextPrime(pd, allocator);
        defer if (comptime ap) pd_next.deinit();
    ...
    if (comptime ap) n.deinit();
    if (comptime ap) pd.deinit();
...
pub fn isMersenne(p: anytype, allocator: std.mem.Allocator) !bool {
    ...
    const T = @TypeOf(p);
    const ap = T == std.math.big.int.Managed;
    const p32 = if (comptime ap) try p.toInt(u32) else @as(u32, @intCast(p));

    var M = try std.math.big.int.Managed.initSet(allocator, 2);
    defer M.deinit();

    try M.shiftLeft(&M, p32 - 1);
    try M.addScalar(&M, -1);
...
pub fn lucasLehmer(M: *const std.math.big.int.Managed, p: u32, allocator: std.mem.Allocator) !bool {
    ...
    var s = try std.math.big.int.Managed.initSet(allocator, 4);
    defer s.deinit();

    var q = try std.math.big.int.Managed.initSet(allocator, 0);
    defer q.deinit();
    ...
    for (0..p - 2) |_| {
        try s.sqr(&s);
        try q.shiftRight(&s, p);
        try s.truncate(&s, .unsigned, p);
        try s.add(&s, &q);
...
pub fn trialDivision(p: anytype, allocator: std.mem.Allocator) !bool {
    ...
    var div = if (comptime ap) try std.math.big.int.Managed.initSet(allocator, 3) else @as(T, 3);
    defer if (comptime ap) div.deinit();

    var dsq = if (comptime ap) try std.math.big.int.Managed.initSet(allocator, 9) else @as(T, 9);
    defer if (comptime ap) dsq.deinit();
