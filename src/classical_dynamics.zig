        ...
        pub const InitialConditions = union(enum) {
            model: struct {
                pub const Spread = struct {
                    momentum_mean: ?struct { end: []const T, step: []const T } = null,
                    position_mean: ?struct { end: []const T, step: []const T } = null,
                    gamma_mean: ?struct { end: []const T, step: []const T } = null,
                };
...
pub fn Custom(comptime T: type) type {
    return struct {
        ...
        pub const TrajectoryOutput = struct {
            pub const Thermodynamics = struct { schlitter_entropy: ?T = null, qha_entropy: ?T = null };
...
pub fn run(comptime T: type, raw_options: Options(T), enable_printing: bool, allocator: std.mem.Allocator) !Output(T) {
    ...
    const q0 = opt.initial_conditions.model.position_mean;
    var q1: []const T = undefined;
    const p0 = opt.initial_conditions.model.momentum_mean;
    var p1: []const T = undefined;
    const g0 = opt.initial_conditions.model.gamma_mean;
    ...
    for (0..qsteps) |i| for (0..psteps) |j| for (0..gsteps) |k| {
        ...
        var options = opt;
        options.initial_conditions.model.position_mean = q;
        options.initial_conditions.model.momentum_mean = p;
        options.initial_conditions.model.gamma_mean = g;

        if (qsteps != 1 or psteps != 1 or gsteps != 1) {
            try renameOutputFilesWithPositionAndMomentum(
                T,
                &options,
...
pub fn performDynamics(comptime T: type, opt: Options(T), enable_printing: bool, allocator: std.mem.Allocator) !Output(T) {
    ...
    var trajectory_based_results = .{ .schlitter_entropy = try RealVector(T).initZero(schlitter_entropy_rows, allocator), .qha_entropy = try RealVector(T).initZero(qha_entropy_rows, allocator) };
...
pub fn runTrajectory(comptime T: type, opt: Options(T), system: *ClassicalParticle(T), index: usize, equilibrate: bool, enable_printing: bool, allocator: std.mem.Allocator) !Custom(T).TrajectoryOutput {
    ...
    var diabatic_potential = try RealMatrix(T).init(nstate, nstate, allocator);
    defer diabatic_potential.deinit(allocator);

    var adiabatic_potential = try RealMatrix(T).init(nstate, nstate, allocator);
    defer adiabatic_potential.deinit(allocator);

    var adiabatic_eigenvectors = try RealMatrix(T).init(nstate, nstate, allocator);
    defer adiabatic_eigenvectors.deinit(allocator);

    var previous_eigenvectors = try RealMatrix(T).init(nstate, nstate, allocator);
    defer previous_eigenvectors.deinit(allocator);

    var eigenvector_overlap = try RealMatrix(T).init(nstate, nstate, allocator);
    defer eigenvector_overlap.deinit(allocator);

    var time_derivative_coupling = try RealMatrix(T).initZero(nstate, nstate, allocator);
    defer time_derivative_coupling.deinit(allocator);

    var energy_gaps = try RingBufferArray(T).init(nstate * (nstate - 1) / 2, .{ .max_len = 5 }, allocator);
    defer energy_gaps.deinit(allocator);

    var jump_probabilities = try RealVector(T).init(nstate, allocator);
    defer jump_probabilities.deinit(allocator);

    var runge_kutta_solver = try ComplexRungeKutta(T).init(nstate, allocator);
    defer runge_kutta_solver.deinit(allocator);

    var coefficient = try ComplexVector(T).initZero(nstate, allocator);
    defer coefficient.deinit(allocator);
    ...
    if (nstate == 2) bloch_vector.ptr(2).* = coefficient.at(1).squaredMagnitude() - coefficient.at(0).squaredMagnitude();
    ...
    if (opt.surface_hopping != null and opt.surface_hopping.? == .mapping_approach) {
        ...
        output.Wcp = 2;
        output.Wpp = 2 * @abs(bloch_vector.at(2));
    }
    ...
    for (0..if (equilibrate) opt.equilibration_iterations + 1 else opt.iterations + 1) |i| {
        ...
        derivative_coupling_parameters.nacv_parameters.time = time;
        ...
        if (opt.write.state_potential_energy_mean != null and !equilibrate) {
            for (0..nstate) |j| output.state_potential_energy.ptr(i, j).* = adiabatic_potential.at(j, j);
        }
        ...
        if (opt.surface_hopping) |algorithm| if (!equilibrate and i > (if (algorithm == .landau_zener) @as(usize, 1) else @as(usize, 0))) {
            ...
            if (new_state != current_state) {
                ...
                potential_energy = adiabatic_potential.at(current_state, current_state);
            }
        ...
        const temperature = system.kineticTemperature();
        ...
        if ((opt.write.position_mean != null or entropy) and !equilibrate) {
            for (0..ndim) |j| output.position.ptr(i, j).* = system.position.at(j);
        }
        if ((opt.write.momentum_mean != null or entropy) and !equilibrate) {
            for (0..ndim) |j| output.momentum.ptr(i, j).* = system.velocity.at(j) * system.masses.at(j);
        }
        ...
        if (opt.cap) |cap| if (!equilibrate) {
            ...
            if (end_simulation) {
                output.final_population.ptr(current_state).* = 1;
                try output.shrink(i, allocator);
                break;
            }
        };
...
pub fn runTrajectoryParallel(id: usize, comptime T: type, results: anytype, trajectory_based_results: anytype, params: anytype, error_ctx: *ErrorContext) void {
    ...
    inline for (std.meta.fields(@TypeOf(results))) |field| {
        const result = @as(field.type, @field(results, field.name)).ptr(id - 1);
        const output = @field(trajectory_output, field.name[0 .. field.name.len - 5]);
...
test "Fewest Switches Surface Hopping on Tully's First Potential" {
    const opt = Options(f64){
        .derivative_coupling = .{ .npi = NormPreservingInterpolation(f64){} },
        ...
        .surface_hopping = .{ .fewest_switches = FewestSwitches(f64){} },
        .iterations = 3500,
        .time_step = 1,
        .trajectories = 1000,
    };
...
test "Mapping Approach to Surface Hopping on Tully's First Potential" {
    const opt = Options(f64){
        ...
        .potential = .{ .tully_1 = TullyPotential1(f64){} },
        .surface_hopping = .{ .mapping_approach = MappingApproach(f64){} },
        .iterations = 3500,
        .time_step = 1,
        .trajectories = 1000,
    };
