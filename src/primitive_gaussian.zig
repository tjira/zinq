...
pub fn PrimitiveGaussian(comptime T: type) type {
    return struct {
        ...
        pub fn evaluate(self: PrimitiveGaussian(T), x: T, y: T, z: T) T {
            const cx = self.center[0];
            const cy = self.center[1];
            const cz = self.center[2];
            const ax = self.angular[0];
            const ay = self.angular[1];
            const az = self.angular[2];
        ...
        pub fn coulomb(self: PrimitiveGaussian(T), other1: PrimitiveGaussian(T), other2: PrimitiveGaussian(T), other3: PrimitiveGaussian(T)) T {
            const p = self.alpha + other1.alpha;
            const q = other2.alpha + other3.alpha;
            const beta = p * q / (p + q);
            var e: T = 0;
        ...
        pub fn hermite_coef(ij: [2]T, ab: [2]T, Q: T, t: T) T {
            ...
            if (ij[0] == 0 and ij[1] == 0 and t == 0) {
                return std.math.exp(-q * Q * Q);
            } else if (ij[0] > 0) {
                ...
                for (0..@as(usize, @intFromFloat(self.angular[0] + other.angular[0])) + 1) |t| {
                    ...
                    for (0..@as(usize, @intFromFloat(self.angular[1] + other.angular[1])) + 1) |u| {
                        ...
                        for (0..@as(usize, @intFromFloat(self.angular[2] + other.angular[2])) + 1) |v| {
                            ...
                            n -= @as(T, @floatFromInt(system.atoms.?[i])) * Eij * Ekl * Emn * hermite_integral(
                                [3]T{ @floatFromInt(t), @floatFromInt(u), @floatFromInt(v) },
                                RPC,
