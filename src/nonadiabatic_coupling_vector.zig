    ...
    return struct {
        ...
        pub fn evaluate(_: @This(), derivative_coupling: *RealMatrix(T), parameters: Parameters(T)) !void {
            ...
            for (0..position.len) |i| {
                ...
                @constCast(&position).ptr(i).* = original_position + FINITE_DIFFERENCES_STEP; 
                ...
                for (0..derivative_coupling.rows) |j| for (j + 1..derivative_coupling.cols) |k| for (0..derivative_coupling.cols) |l| {
                    ...
                    const bra = adiabatic_eigenvectors.at(l, j); const ket = (eigenvectors_plus.at(l, k) - eigenvectors_minus.at(l, k)) / (2 * FINITE_DIFFERENCES_STEP);
