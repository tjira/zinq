...
pub fn DftEvaluationContext(comptime T: type) type {
    return struct {
...
pub fn DftPotential(comptime T: type) type {
    return struct {
        ...
        pub fn getFamilyFlags(self: @This()) struct { has_sgga: bool, has_mgga: bool, needs_deriv: bool } {
        ...
        pub fn computeDensity(self: @This(), P: Matrix(T), ctx: *DftEvaluationContext(T)) void {
            for (0..self.pts.shape[0]) |i| {
                if (self.polarized) {
                    var rho_a: T = 0;
                    var rho_b: T = 0;
        ...
        pub fn integratePotential(self: *@This(), ctx: DftEvaluationContext(T), gpa: Allocator) !void {
            var energy_exc: T = 0;
            ...
            for (0..self.pts.shape[0]) |i| {
                ...
                if (self.polarized) {
                    const rho_tot = ctx.density.rho_val.at(i, 0) + ctx.density.rho_val.at(i, 1);

                    energy_exc += rho_tot * ctx.potgd.exc.at(i) * w;
