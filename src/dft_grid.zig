...
pub fn Uniform(comptime T: type) type {
    return struct {
        ...
        pub fn get(self: @This(), _: BasisSet(T), allocator: std.mem.Allocator) !struct { RealMatrix(T), RealVector(T) } {
            var points = try RealMatrix(T).init(self.points * self.points * self.points, 3, allocator);
            errdefer points.deinit(allocator);

            var weights = try RealVector(T).init(self.points * self.points * self.points, allocator);
    ...
    return struct {
        ...
        pub fn get(self: @This(), basis: BasisSet(T), allocator: std.mem.Allocator) !struct { RealMatrix(T), RealVector(T) } {
            ...
            for (basis.contracted_gaussians) |cg| {
                ...
                if (is_unique) {
                    try centers.append(allocator, cg.center);
                    try atomic_numbers.append(allocator, cg.atomic_number);
                }
            }

            const total_points = centers.items.len * self.n_radial * self.n_lebedev;

            var points = try RealMatrix(T).init(total_points, 3, allocator);
            errdefer points.deinit(allocator);

            var weights = try RealVector(T).init(total_points, allocator);
            errdefer weights.deinit(allocator);

            var W_vals = try allocator.alloc(T, centers.items.len);
            defer allocator.free(W_vals);

            var R_AB = try allocator.alloc(T, centers.items.len * centers.items.len);
            defer allocator.free(R_AB);

            var a_AB = try allocator.alloc(T, centers.items.len * centers.items.len);
            defer allocator.free(a_AB);
            ...
            for (0..centers.items.len) |A| for (0..self.n_radial) |ir| {
                ...
                for (lebedev_points.nodes, lebedev_points.weights) |lebedev_node, lebedev_weight| {
                    ...
                    for (0..centers.items.len) |I| {
                        ...
                        const r_I = std.math.sqrt(std.math.pow(T, points.at(idx, 0) - centers.items[I][0], 2) +
                        ...
                        for (0..centers.items.len) |J| {
                            ...
                            const r_J = std.math.sqrt(std.math.pow(T, points.at(idx, 0) - centers.items[J][0], 2) +
                    ...
                    weights.ptr(idx).* = 4 * std.math.pi * w_r * lebedev_weight * W_vals[A] / P_sum;
                    idx += 1;
                }
            };
