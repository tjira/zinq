...
pub fn ci(comptime T: type, opt: Options(T), system: ClassicalParticle(T), enable_printing: bool, allocator: std.mem.Allocator) !Output(T) {
    ...
    var AS = [2]usize{ nocc, nbf };
    if (opt.active_space) |as| {
        AS[0] = as[0];
        AS[1] = as[1];
    }
    ...
    var H_AO = try hf_output.K.clone(allocator);
    defer H_AO.deinit(allocator);
    try H_AO.add(hf_output.V);

    var H_MS = try RealMatrix(T).init(nbf, nbf, allocator);
    defer H_MS.deinit(allocator);
    ...
    var H = try RealMatrix(T).init(D.rows, D.rows, allocator);
    defer H.deinit(allocator);

    var E = try RealMatrix(T).init(D.rows, D.rows, allocator);
    defer E.deinit(allocator);

    var C = try RealMatrix(T).init(D.rows, D.rows, allocator);
    defer C.deinit(allocator);

    H.zero();

    for (0..D.rows) |i| for (i..D.rows) |j| {
        var so: [4]usize = undefined;
        var diff: u32 = 0;
        var k: usize = 0;
        ...
        if (diff == 2) for (0..nocc) |l| if (A.at(l) != D.at(i, l)) {
            so[if (k == 0) 0 else 1] = A.at(l);
            so[if (k == 0) 2 else 3] = D.at(i, l);
            k += 1;
        };

        H.ptr(i, j).* = @as(T, @floatFromInt(sign)) * try slater(T, A, so[0 .. diff * 2], H_MS, J_MS);
        H.ptr(j, i).* = H.at(i, j);
    };
...
fn alignDeterminant(A: *RealVector(usize), B: RealVector(usize), C: RealVector(usize)) !i32 {
    ...
    while (k < A.len) : (k += 1) {
        if (A.at(@intCast(k)) != B.at(@intCast(k))) for (@as(usize, @intCast(k)) + 1..A.len) |l| if (A.at(@intCast(k)) == B.at(l) or A.at(l) == B.at(@intCast(k))) {
            std.mem.swap(usize, A.ptr(@intCast(k)), A.ptr(l));
            sign *= -1;
            k -= 1;
            break;
        };
