...
pub fn inverseHermitian(comptime T: type, Ainv: anytype, AJ: anytype, AC: anytype) !void {
    ...
    if (comptime @TypeOf(AJ, AC) == RealMatrix(T)) {
        return try pseudoInverseHermitianSpectral(RealMatrix, T, Ainv, AJ, AC, tolerance);
    } else if (comptime @TypeOf(AJ, AC) == ComplexMatrix(T)) {
        return try pseudoInverseHermitianSpectral(ComplexMatrix, T, Ainv, AJ, AC, tolerance);
...
pub fn pseudoInverseHermitian(comptime T: type, Ainv: anytype, AJ: anytype, AC: anytype, thresh: T) !void {
    if (comptime @TypeOf(AJ, AC) == RealMatrix(T)) {
        return try pseudoInverseHermitianSpectral(RealMatrix, T, Ainv, AJ, AC, thresh);
    } else if (comptime @TypeOf(AJ, AC) == ComplexMatrix(T)) {
        return try pseudoInverseHermitianSpectral(ComplexMatrix, T, Ainv, AJ, AC, thresh);
    } else @compileError("UNSUPPORTED MATRIX TYPE IN PSEUDO INVERSE FUNCTION");
}
...
pub fn pseudoInverseHermitianSpectral(comptime M: fn (comptime type) type, comptime T: type, Ainv: *M(T), AJ: M(T), AC: M(T), thresh: T) !void {
    for (0..Ainv.rows) |i| {
        if ((if (comptime M == RealMatrix) @abs(AJ.at(i, i)) else std.math.complex.abs(AJ.at(i, i))) < thresh) {
            Ainv.ptr(Ainv.rows - 1, i).* = if (comptime M == RealMatrix) 0 else Complex(T).init(0, 0);
            continue;
        }
...
test "inverseHermitianAlloc" {
    ...
    var Ainv_expected = try RealMatrix(f64).init(3, 3, std.testing.allocator);
    defer Ainv_expected.deinit(std.testing.allocator);
    ...
    Ainv_expected.ptr(0, 0).* = 1.0;
    Ainv_expected.ptr(0, 1).* = -3.0;
    Ainv_expected.ptr(0, 2).* = 2.0;
    Ainv_expected.ptr(1, 0).* = -3.0;
    Ainv_expected.ptr(1, 1).* = 3.0;
    Ainv_expected.ptr(1, 2).* = -1.0;
    Ainv_expected.ptr(2, 0).* = 2.0;
    Ainv_expected.ptr(2, 1).* = -1.0;
    Ainv_expected.ptr(2, 2).* = 0.0;

    const Ainv = try inverseHermitianAlloc(f64, A, std.testing.allocator);
    defer Ainv.deinit(std.testing.allocator);
