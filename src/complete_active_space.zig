...
pub fn generateAllDeterminants(nbf: usize, nel: usize, allocator: std.mem.Allocator) !RealMatrix(usize) {
    ...
    for (1..D.rows) |i| {
        var row_i = D.row(i);
        try D.row(i - 1).copyTo(&row_i);
        var index: usize = 0;

        for (0..D.cols) |j| if (D.at(i, D.cols - j - 1) != nbf - j - 1) {
            D.ptr(i, D.cols - j - 1).* += 1;
            index = D.cols - j - 1;
            break;
        };
...
pub fn generateCasDeterminants(nel: usize, nbf: usize, nocc: usize, spin: bool, allocator: std.mem.Allocator) !RealMatrix(usize) {
    const CASD = if (spin) try generateAllDeterminants(nbf, nel, allocator) else try generateClosedShellDeterminants(nbf, nel, allocator);
    defer CASD.deinit(allocator);
    ...
    for (0..CASD.rows) |i| {
        ...
        for (0..CASD.cols) |j| D.ptr(i, j + nocc - nel).* = CASD.at(i, j) + nocc - nel;
    }
...
pub fn generateClosedShellDeterminants(nbf: usize, nel: usize, allocator: std.mem.Allocator) !RealMatrix(usize) {
    const data_alpha = try allocator.alloc(usize, nbf / 2);
    defer allocator.free(data_alpha);

    const data_beta = try allocator.alloc(usize, nbf / 2);
    defer allocator.free(data_beta);

    for (0..data_alpha.len) |i| data_alpha[i] = 2 * i + 0;
    for (0..data_beta.len) |i| data_beta[i] = 2 * i + 1;

    var dets_alpha = try combinations(usize, data_alpha, nel / 2, allocator);
    defer dets_alpha.deinit(allocator);

    var dets_beta = try combinations(usize, data_beta, nel / 2, allocator);
    defer dets_beta.deinit(allocator);
