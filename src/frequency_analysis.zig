...
pub fn particleHarmonicFrequencies(comptime T: type, system: ClassicalParticle(T), hessian: RealMatrix(T), allocator: std.mem.Allocator) !RealVector(T) {
    var freqs = try RealVector(T).init(hessian.rows, allocator);
    var HM = try hessian.clone(allocator);
    ...
    const HMJC = try eigensystemHermitianAlloc(T, HM, allocator);
    defer HMJC.J.deinit(allocator);
    defer HMJC.C.deinit(allocator);
