...
pub fn BasisSet(comptime T: type) type {
    return struct {
        ...
        pub fn init(system: ClassicalParticle(T), name: []const u8, allocator: std.mem.Allocator) !BasisSet(T) {
            var basis: std.ArrayList(ContractedGaussian(T)) = .empty;
            errdefer basis.deinit(allocator);
            ...
            const basis_json = try std.json.parseFromSlice(std.json.Value, allocator, basis_file_contents, .{});
            defer basis_json.deinit();
            ...
            for (0..system.atoms.?.len) |i| {
                ...
                for (shells) |shell| {
                    ...
                    var alpha = try allocator.alloc(T, exponents.len);
