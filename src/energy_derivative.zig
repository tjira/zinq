...
pub fn firstPartialDerivative(comptime T: type, result: *T, opt: anytype, system: ClassicalParticle(T), efunc: anytype, i: usize, enable_printing: bool, allocator: std.mem.Allocator) !void {
    ...
    var outp1 = try efunc(T, opt, sysp1, false, allocator);
    const Ep1 = outp1.energy;
    outp1.deinit(allocator);

    var outm1 = try efunc(T, opt, sysm1, false, allocator);
...
pub fn nuclearGradient(comptime T: type, opt: anytype, system: ClassicalParticle(T), efunc: anytype, method: []const u8, enable_printing: bool, allocator: std.mem.Allocator) !RealMatrix(T) {
    ...
    try pool.init(.{ .n_jobs = opt.gradient.?.numeric.nthread, .allocator = allocator });
    ...
    for (0..system.position.len) |i| {
        const params = .{ opt, system, efunc, i, enable_printing, allocator };
...
pub fn nuclearHessian(comptime T: type, opt: anytype, system: ClassicalParticle(T), efunc: anytype, method: []const u8, enable_printing: bool, allocator: std.mem.Allocator) !RealMatrix(T) {
    ...
    var pool: std.Thread.Pool = undefined;
    try pool.init(.{ .n_jobs = opt.hessian.?.numeric.nthread, .allocator = allocator });
    var error_ctx = ErrorContext{};
    ...
    for (0..hess.rows) |i| for (i..hess.cols) |j| {
        const params = .{ opt, system, efunc, i, j, enable_printing, allocator };
...
pub fn secondPartialDerivative(comptime T: type, result: *T, opt: anytype, system: ClassicalParticle(T), efunc: anytype, i: usize, j: usize, enable_printing: bool, allocator: std.mem.Allocator) !void {
    ...
    var outp2 = try efunc(T, opt, sysp2, false, allocator);
    const Ep2 = outp2.energy;
    outp2.deinit(allocator);

    var outp1m1 = try efunc(T, opt, sysp1m1, false, allocator);
    const Ep1m1 = outp1m1.energy;
    outp1m1.deinit(allocator);

    var outm1p1 = try efunc(T, opt, sysm1p1, false, allocator);
    const Em1p1 = outm1p1.energy;
    outm1p1.deinit(allocator);

    var outm2 = try efunc(T, opt, sysm2, false, allocator);
