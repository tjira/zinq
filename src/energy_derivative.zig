...
pub fn nuclearGradient(comptime T: type, opt: anytype, system: ClassicalParticle(T), efunc: anytype, method: []const u8, enable_printing: bool, allocator: std.mem.Allocator) !RealMatrix(T) {
    ...
    for (0..system.position.len) |i| {
        ...
        if (opt.gradient.?.numeric.nthread > 1) {
            try pool.spawn(parallel_function, .{grad.ptr(i / 3, i % 3), .{opt, system, efunc, i, enable_printing, allocator}, &parallel_error});
        } else {
            try firstPartialDerivative(T, grad.ptr(i / 3, i % 3), opt, system, efunc, i, enable_printing, allocator);
        }
    }
...
pub fn nuclearHessian(comptime T: type, opt: anytype, system: ClassicalParticle(T), efunc: anytype, method: []const u8, enable_printing: bool, allocator: std.mem.Allocator) !RealMatrix(T) {
    ...
    for (0..hess.rows) |i| for (i..hess.cols) |j| {
        ...
        if (opt.hessian.?.numeric.nthread > 1) {
            try pool.spawn(parallel_function, .{hess.ptr(i, j), .{opt, system, efunc, i, j, enable_printing, allocator}, &parallel_error});
        } else {
            try secondPartialDerivative(T, hess.ptr(i, j), opt, system, efunc, i, j, enable_printing, allocator);
        }
    };
