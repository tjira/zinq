...
const gradient = energy_derivative.gradient;
const throw = error_handling.throw;
const print = device_write.print;

/// Optimizes the positions of a set of classical particles using the steepest descent method and a gradient.
pub fn steepestDescent(comptime T: type, opt: anytype, system: ClassicalParticle(T), efunc: anytype, method: []const u8, enable_printing: bool, allocator: std.mem.Allocator) !ClassicalParticle(T) {
    ...
    for (0..opt.optimize.?.maxiter + 1) |i| {
        ...
        var G = try gradient(T, opt, optimized_system, efunc, method, false, allocator);
