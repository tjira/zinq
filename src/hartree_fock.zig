...
fn orbitalResponse(comptime T: type, io: std.Io, hfres: Result(T), opt: anytype, log: bool, gpa: Allocator) !struct { Tensor(T, 3), Matrix(T) } {
...
fn solveCPHF(comptime T: type, io: std.Io, dC: *Tensor(T, 3), de: *Matrix(T), hfres: Result(T), opt: anytype, log: bool, gpa: Allocator) !void {
