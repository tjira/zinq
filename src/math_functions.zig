...
pub fn combinations(comptime T: type, array: []const T, n: usize, allocator: std.mem.Allocator) !std.ArrayList([]T) {
    ...
    const Backtrack = struct {
        fn get(res: *std.ArrayList([]T), arr: []const T, m: usize, curr: *std.ArrayList(usize), start: usize, alloc: std.mem.Allocator) !void {
            if (curr.items.len == m) {
                ...
                try res.append(alloc, last);
                return;
            }

            for (start..arr.len) |i| {
                try curr.append(alloc, i);
                try get(res, arr, m, curr, i + 1, alloc);
                _ = curr.pop();
            }
        }
    };

    try Backtrack.get(&result, array, n, &current, 0, allocator);
    return result;
}
...
pub fn powMod(base: anytype, exp: anytype, m: anytype) @TypeOf(base, exp, m) {
    var result: @TypeOf(base, exp, m) = 1;
    var b = base % m;
    var e = exp;
