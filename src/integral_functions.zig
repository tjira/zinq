...
pub fn lnGamma(z: anytype) @TypeOf(z) {
    var zbase = z;
    var phase = @TypeOf(z).init(0, 0);
    const one = @TypeOf(z).init(1, 0);

    while (zbase.re < 7) {
        phase = phase.sub(std.math.complex.log(zbase));
        zbase = zbase.add(@TypeOf(z).init(1, 0));
    }

    const lnzbase = std.math.complex.log(zbase);
    const ln2pi = @TypeOf(z).init(std.math.log(@TypeOf(z.re), std.math.e, 2 * std.math.pi), 0);
