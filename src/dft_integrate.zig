...
pub fn DensityIntegrateContext(comptime T: type) type {
    return struct {
        basis: BasisSet(T), points: RealMatrix(T), weights: RealVector(T), functional: DensityFunctional(T), generalized: bool
    };
}
...
pub fn evaluateXC(comptime T: type, Vxc: *RealMatrix(T), P: RealMatrix(T), context: DensityIntegrateContext(T), allocator: std.mem.Allocator) !T {
    ...
    var eps_x = try RealVector(T).initZero(points.rows, allocator); defer eps_x.deinit(allocator);
    var eps_c = try RealVector(T).initZero(points.rows, allocator); defer eps_c.deinit(allocator);
    var vxc_x = try RealVector(T).initZero(points.rows, allocator); defer vxc_x.deinit(allocator);
    var vxc_c = try RealVector(T).initZero(points.rows, allocator); defer vxc_c.deinit(allocator);
...
pub fn evaluateXCKernel(comptime T: type, K: *RealMatrix(T), P: RealMatrix(T), C: RealMatrix(T), context: DensityIntegrateContext(T), nocc: usize, allocator: std.mem.Allocator) !void {
    ...
    var fxc_x = try RealVector(T).initZero(points.rows, allocator); defer fxc_x.deinit(allocator);
    var fxc_c = try RealVector(T).initZero(points.rows, allocator); defer fxc_c.deinit(allocator);
