...
pub fn build(builder: *std.Build) !void {
    ...
    const docs_target = builder.addInstallDirectory(.{ .source_dir = builder.addLibrary(.{
        .name = "main",
        .root_module = main_executable.root_module,
    }).getEmittedDocs(), .install_dir = .prefix, .install_subdir = "../docs/code" });

    main_executable.root_module.addOptions("config", main_options);

    try install(builder, main_executable, builder.getInstallStep(), link_c);
    try addTools(builder, main_executable, builder.getInstallStep(), link_c);
    ...
    for (0..targets.len) |i| {
        ...
        try install(builder, matrix_executable, cross, false);
        try addTools(builder, matrix_executable, cross, false);
...
pub fn addLibcLinkedExecutables(builder: *std.Build, optimize: std.builtin.OptimizeMode, target_query: std.Target.Query, step: *std.Build.Step, use_libint: bool, use_openblas: bool, use_xc: bool) !void {
    ...
    try install(builder, libc_linked_executable, step, true);
    try addTools(builder, libc_linked_executable, step, true);
}
...
pub fn install(builder: *std.Build, main_executable: *std.Build.Step.Compile, step: *std.Build.Step, libc: bool) !void {
    ...
    const dest = try std.fmt.allocPrint(builder.allocator, "{s}-{s}{s}", .{ @tagName(target.result.cpu.arch), @tagName(target.result.os.tag), if (libc) "-libc" else "" });

    const main_executable_install = builder.addInstallArtifact(main_executable, .{ .dest_dir = .{ .override = .{ .custom = dest } } });
